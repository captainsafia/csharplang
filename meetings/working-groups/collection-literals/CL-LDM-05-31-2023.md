...
public struct ImmutableArray<T> : IEnumerable<T> { } // or

[CollectionLiteralBuilder(
    typeof(CollectionsMarshal),
    nameof(CollectionsMarshal.CreateRange))] 
public sealed class ImmutablelDictionary<TKey, TValue> : IDictionary<TKey, TValue> { }
...
2. How do we recognize dictionary types (`IDictionary`, `IDictionary<K,V>`, or pattern)?  When using interfaces, do we guarantee a particular concrete type?
...
1. Should we use AddRange if available? What if it boxes? We could leave it underspecified and utilized compiler heuristics. Fits into the overal discussion of the presumption that "construction" semantics are well-behaved (like pattern matching semantics). 
